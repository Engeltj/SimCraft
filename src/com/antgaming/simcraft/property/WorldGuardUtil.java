...
//        }
    ...
    public ProtectedRegion createBuildRegion(String player, Location start, Location end) throws Exception{
        ...
        RegionManager mgr = wgp.getRegionContainer().get(start.getWorld());
        mgr.removeRegion(id);
    ...
    public ProtectedRegion createRegionFromSelection(Player p, String rgName){
        ...
        RegionManager mgr = wgp.getRegionContainer().get(p.getWorld());
    ...
    public void deleteRegion(String region, World w) {
    ...
    public com.sk89q.worldedit.entity.Player getPlayer(Player p){
        BukkitServerInterface bsi = new BukkitServerInterface(wep, Bukkit.getServer());
...
//        player.print("The clipboard has been pasted at " + to);
    ...
    public boolean pasteSchematic(Player p, ProtectedRegion pr, String schematic, String subdir) {
        ...
        try {
            ...
            clearRegion(pr, p.getWorld().getName());
            return true;
        } catch (Exception ex) {
            SimUtil.sendConsole("Failed to pasteSchematic() of name '"+schematic+"', "+ex);
        }
        return false;
    }
...
//        return (same/total*100D);
    ...
    public boolean saveSchematic(Player p, ProtectedRegion region, String subdir, String schematicName){
        ...
        try {
            ...
            SimUtil.sendConsole("Failed to saveSchematic of name '" + schematicName +"', reason: " + ex);
            return false;
        }
    }
...
//        return false;
    ...
    private void save(CommandContext args, LocalSession session, Player p, String subdir) throws WorldEditException, CommandException {
        ...
        File dir = new File(SimUtil.getDataFolder() + File.separator + "schematics" + File.separator + subdir);
        File f = we.getSafeSaveFile(p_wep, dir, filename, "schematic", "schematic");
        if (!dir.exists()) {
            if (!dir.mkdirs()) {
                p_wep.printError(SimUtil.getDataFolder() + File.separator + "schematics" + File.separator + subdir);
        ...
        try {
            ...
            SimUtil.sendConsole(p.getName() + " saved " + f.getCanonicalPath());
